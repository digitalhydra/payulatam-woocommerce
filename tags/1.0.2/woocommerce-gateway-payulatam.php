	...
	class WC_payulatam extends WC_Payment_Gateway{
		public function __construct(){
			...
			if ( $this->addcop == "yes" ) {
				...
				add_filter( 'woocommerce_currency_symbol', 'add_all_symbol', 1, 2);
			} 
		...
		function init_form_fields(){
			$this->form_fields = array(
				'enabled' => array(
      			...
      			'addcop' => array(
					'title' 		=> __('Add Currencys Supported by PayU Latam', 'kdc'),
					'type' 			=> 'checkbox',
					'label' 		=> __('Add 5 more Currencys for Latam in currency settings.', 'kdc'),
					'default' 		=> 'no',
					'description' 	=> __('Add ARS(Peso Argentino), BRL(Real Brasileño), COP(Colombian Pesos), MXN(Peso Mexicano), PEN(Nuevo Sol Peruano) available in currency settings'),
					'desc_tip' 		=> true
                ),
                'taxes' => array(
							'title' 		=> __('Tax Rate'),
							'type' 			=> 'text',
							'default' 		=> __('0', 'kdc'),
							'description' 	=> __('Tax rates for Transactions (IVA). Read <a target="_blank" href="http://docs.payulatam.com/manual-integracion-web-checkout/informacion-adicional/tablas-de-variables-complementarias/">PayU Documentacion</a>', 'kdc'),
							'desc_tip' 		=> true
		        ),
      			'tax_return_base' => array(
					...
					'default' 		=> '0',
					'description' 	=> __('Tax base to calculate IVA ', 'kdc'),
					'desc_tip' 		=> true
                ),
      			'payu_language' => array(
		...
		**/
		        ...
		        if (!empty($posted['transactionState'])) {
			        ...
			        switch ( $posted['lapTransactionState'] ) {
			            ...
			            case 'CREDIT_CARD_NOT_AUTHORIZED_FOR_INTERNET_TRANSACTIONS' :
			                // Order failed
			                $order->update_status( 'failed', sprintf( __( 'Payment rejected via PayU Latam. Error type: %s', 'woocommerce' ), ( $posted['lapTransactionState'] ) ) );
								$this->msg['message'] = sprintf( __( 'Payment rejected via PayU Latam. Error type: %s.', 'woocommerce' ), ( $posted['lapTransactionState'] ) )  ;
								$this->msg['class'] = 'woocommerce-error';
			            break;
			            default :
			                $order->update_status( 'failed', sprintf( __( 'Payment rejected via PayU Latam. Error type: %s', 'woocommerce' ), ( $posted['lapTransactionState'] ) ) );
			        ...
			        switch ( $codes[$state] ) {
			            ...
			            case 'CAPTURING_DATA' :
			                ...
			                if ( $codes[$state] == 'APPROVED' ) {
			                	...
			                	$order->update_status( 'on-hold', sprintf( __( 'Payment pending: %s', 'woocommerce' ), $codes[$state] ) );
								$this->msg['message'] = sprintf( __( 'Payment pending: %s', 'woocommerce' ), $codes[$state] ) ;
								$this->msg['class'] = 'woocommerce-info';
			                }
			            ...
			            case 'ERROR' :
			                // Order failed
			                $order->update_status( 'failed', sprintf( __( 'Payment rejected via PayU Latam. Error type: %s', 'woocommerce' ), ( $codes[$state] ) ) );
								$this->msg['message'] = sprintf( __( 'Payment rejected via PayU Latam. Error type: %s.', 'woocommerce' ), ( $codes[$state] ) )  ;
								$this->msg['class'] = 'woocommerce-error';
			            break;
			            default :
			                $order->update_status( 'failed', sprintf( __( 'Payment rejected via PayU Latam.', 'woocommerce' ), ( $codes[$state] ) ) );
								$this->msg['message'] = sprintf( __( 'Payment rejected via PayU Latam.', 'woocommerce' ), ( $codes[$state] ) )  ;
								$this->msg['class'] = 'woocommerce-error';
			            break;
			        }
		        }
		...
		function add_all_currency( $currencies ) {
			$currencies['ARS'] = __( 'Peso Argentino', 'woothemes' );
			$currencies['BRL'] = __( 'Real Brasileño', 'woothemes' );
			$currencies['COP'] = __( 'Pesos Colombianos', 'woothemes' );
			$currencies['MXN'] = __( 'Peso Mexicano', 'woothemes' );
			$currencies['PEN'] = __( 'Nuevo Sol Peruano', 'woothemes' );
			return $currencies;
		}
